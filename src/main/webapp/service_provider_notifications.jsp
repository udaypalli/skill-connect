...
<%
  ...
  if (request.getMethod().equalsIgnoreCase("POST")) {
    ...
    if (action != null) {
      if (action.equals("deleteAll")) {
        ...
        String deleteAllQuery = "DELETE FROM Notifications WHERE provider_id = ?";
        ...
        if (startDate != null && endDate != null) {
          String deleteRangeQuery = "DELETE FROM Notifications WHERE provider_id = ? AND created_at BETWEEN ? AND ?";
  ...
  // Fetch notifications for the service provider
  List<Map<String, String>> notifications = new ArrayList<>();
  if (providerId != -1) {
    String notificationQuery = "SELECT notification_id, message, created_at FROM Notifications WHERE provider_id = ? AND (message LIKE ? OR message LIKE ?) ORDER BY created_at DESC";
